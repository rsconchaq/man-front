...
function r(e2, t2, i2) {
  let s2 = e2;
  const r2 = Array.isArray(t2) ? t2 : t2.split(".");
  for (let e3 = 0; e3 < r2.length; ++e3) e3 === r2.length - 1 ? s2[r2[e3]] = i2 : s2 = s2[r2[e3]];
}
var n = {
  aliceblue: "#f0f8ff",
  antiquewhite: "#faebd7",
...
function o(e2) {
  if (e2 && "string" == typeof e2) return e2.split(",").filter((e3) => e3).map((e3) => e3.split(":")).filter((e3) => 2 == e3.length).reduce((e3, t2) => (e3[t2[0].toLowerCase()] = a(t2[1]), e3), {});
}
function l(e2) {
...
function m(e2, {
  ...
  return e2 && e2.layers ? (e2.layers.forEach((e3, s2) => {
    e3.nm && e3.ef && e3.ef.forEach((e4, r2) => {
      const n2 = e4?.ef?.[0]?.v?.k;
      if (void 0 === n2) return;
      let a2, o2;
      if (a2 = t2 ? `renderer.elements.${s2}.effectsManager.effectElements.${r2}.effectElements.0.p.v` : `layers.${s2}.ef.${r2}.ef.0.v.k`, "ADBE Color Control" === e4.mn ? o2 = "color" : "ADBE Slider Control" === e4.mn ? o2 = "slider" : "ADBE Point Control" === e4.mn ? o2 = "point" : "ADBE Checkbox Control" === e4.mn ? o2 = "checkbox" : e4.mn.startsWith("Pseudo/") && (o2 = "feature"), !o2) return;
...
function y(e2, t2) {
  for (const i2 of t2) r(e2, i2.path, i2.value);
}
function p(e2, t2, i2) {
  for (const s2 of t2) "color" === s2.type ? "object" == typeof i2 && "r" in i2 && "g" in i2 && "b" in i2 ? r(e2, s2.path, [d(i2.r), d(i2.g), d(i2.b)]) : Array.isArray(i2) ? r(e2, s2.path, i2) : "string" == typeof i2 && r(e2, s2.path, f(a(i2))) : "point" === s2.type ? "object" == typeof i2 && "x" in i2 && "y" in i2 ? (r(e2, s2.path + ".0", i2.x), r(e2, s2.path + ".1", i2.y)) : Array.isArray(i2) && (r(e2, s2.path + ".0", i2[0]), r(e2, s2.path + ".1", i2[1])) : r(e2, s2.path, i2);
}
...
var I = class _I extends HTMLElement {
  static _iconLoader;
  static _playerFactory;
  static _definedTriggers = /* @__PURE__ */ new Map();
  static get version() {
    return "1.6.0";
  }
  ...
  connectedCallback() {
    if (this._root || this.createElements(), "lazy" === this.loading) {
      ...
      this.delayedLoading = (s2) => {
        for (const e3 of b) (t2 || this).removeEventListener(e3, i2);
        this.delayedLoading = null, s2 || this.createPlayer().then(() => {
          e2 && (t2 || this).dispatchEvent(new Event(e2));
        });
      };
      ...
      i2 = i2.bind(this);
      for (const e3 of b) (t2 || this).addEventListener(e3, i2);
    } else if ("delay" === this.loading) {
      this.delayedLoading = (e3) => {
        this.delayedLoading = null, e3 || this.createPlayer();
      };
      const e2 = this.hasAttribute("delay") ? +this.getAttribute("delay") : 0;
      setTimeout(() => {
  ...
  createElements() {
    if (this._root = this.attachShadow({
      mode: "open"
    }), _) E || (E = new CSSStyleSheet(), E.replaceSync(v)), this._root.adoptedStyleSheets = [E];
    else {
  ...
  createPlayer() {
    return __async(this, null, function* () {
      if (!_I._playerFactory) throw new Error("Missing player loader!");
      if (this.delayedLoading) return;
      const e2 = yield this.loadIconData();
      if (!e2) return;
      this._playerInstance = _I._playerFactory(this.animationContainer, e2, {
        state: h(this.state),
      ...
      }), this.refresh(), this.triggerChanged(), yield new Promise((e3, t3) => {
  ...
  loadIconData() {
    return __async(this, null, function* () {
      ...
      if (!e2) {
        if (this.icon && _I._iconLoader) this._loadedIconData = e2 = yield _I._iconLoader(this.icon);
        else if (this.src) {
          const t2 = yield fetch(this.src);
          this._loadedIconData = e2 = yield t2.json();
        }
      }
      return e2;
    });
  ...
  movePaletteToCssVariables() {
    for (const [e2, t2] of Object.entries(this._playerInstance.colors || {})) t2 ? this.animationContainer.style.setProperty(`--lord-icon-${e2}-base`, t2) : this.animationContainer.style.removeProperty(`--lord-icon-${e2}-base`);
  }
  targetChanged() {
    this.triggerChanged();
  }
  ...
  triggerChanged() {
    ...
    if (!e2) throw new Error("Can't use unregistered trigger!");
    const t2 = this.target ? this.closest(this.target) : null;
  ...
  set icon(e2) {
    if (e2 && i(e2)) this._assignedIconData !== e2 && (this._assignedIconData = e2, this.hasAttribute("icon") ? this.removeAttribute("icon") : this.iconChanged());
    else {
  ...
  get loading() {
    if (this.getAttribute("loading")) {
      ...
      if ("lazy" === e2) return "lazy";
      if ("interaction" === e2) return "interaction";
      if ("delay" === e2) return "delay";
    }
    return null;
  }
  set target(e2) {
...
function L() {
  return new Proxy(this, {
    set: (e2, t2, i2, s2) => ("string" == typeof t2 && (i2 ? p(this.lottie, this.rawProperties.filter((e3) => "color" === e3.type && e3.name === t2), i2) : y(this.lottie, this.rawProperties.filter((e3) => "color" === e3.type && e3.name === t2)), e2.refresh()), true),
    get: (e2, t2, i2) => {
      for (const i3 of e2.rawProperties) if ("color" == i3.type && "string" == typeof t2 && t2 == i3.name) {
        const e3 = s(this.lottie, i3.path);
        if (e3) return u(e3);
      }
    },
    deleteProperty: (e2, t2) => ("string" == typeof t2 && (y(this.lottie, this.rawProperties.filter((e3) => "color" === e3.type && e3.name === t2)), e2.refresh()), true),
    ownKeys: (e2) => e2.rawProperties.filter((e3) => "color" == e3.type).map((e3) => e3.name),
    has: (e2, t2) => {
      for (const i2 of e2.rawProperties) if ("color" == i2.type && "string" == typeof t2 && t2 == i2.name) return true;
      return false;
    },
    getOwnPropertyDescriptor: (e2) => ({
      enumerable: true,
      configurable: true
    })
  });
...
var w = class {
  ...
  constructor(i2, s2, n2, a2, o2) {
    if (this._animationLoader = i2, this._container = s2, this._iconData = n2, this._initial = a2 || {}, this._options = o2 || C, this._states = (n2.markers || []).map((e2) => {
      const [i3, s3] = e2.cm.split(":"), r2 = {
        time: e2.tm,
        duration: e2.dr,
        name: s3 || i3,
        default: !(!s3 || !i3.includes("default"))
      };
      return (r2.name === this._initial.state || r2.default && t(this._initial.state)) && (this._state = r2), r2;
    }).filter((e2) => e2.duration > 0), this._states.length && (this._initial.stroke && ![1, 2, 3, "light", "regular", "bold"].includes(this._initial.stroke) && delete this._initial.stroke, this._initial.state && !this._state && (this._state = this._states.filter((e2) => e2.default)[0])), !this._states.length) {
      this._iconData = e(this._iconData);
      const t2 = m(this._iconData, {
        lottieInstance: false
      });
  ...
  connect() {
    ...
    this._lottie = this._animationLoader(__spreadProps(__spreadValues(__spreadValues({}, this._options), i2), {
      container: this._container,
      animationData: Object.assign(e(this._iconData), t2)
    })), this._initial.colors && (this.colors = this._initial.colors), this._initial.stroke && (this.stroke = this._initial.stroke), this._lottie.addEventListener("complete", (e2) => {
  ...
  removeEventListener(e2, t2) {
    if (t2) {
      if (this._eventCallbacks[e2]) {
        ...
        for (; i2 < s2; ) this._eventCallbacks[e2][i2] === t2 && (this._eventCallbacks[e2].splice(i2, 1), i2 -= 1, s2 -= 1), i2 += 1;
        this._eventCallbacks[e2].length || (this._eventCallbacks[e2] = null);
      }
    } else this._eventCallbacks[e2] = null;
  }
  triggerEvent(e2, t2) {
    if (this._eventCallbacks[e2]) {
      const i2 = this._eventCallbacks[e2];
      for (let e3 = 0; e3 < i2.length; e3 += 1) i2[e3](t2);
    }
  }
  refresh() {
  ...
  get properties() {
    const e2 = {};
    return this.rawProperties.filter((e3) => "color" === e3.type).length && (e2.colors = __spreadValues({}, this.colors)), this.rawProperties.filter((e3) => "stroke" === e3.name || "stroke-layers" === e3.name).length && (e2.stroke = this.stroke), this._states.length && (e2.state = this.state), e2;
  }
  set colors(e2) {
    if (y(this._lottie, this.rawProperties.filter((e3) => "color" === e3.type)), e2) for (const [t2, i2] of Object.entries(e2)) p(this._lottie, this.rawProperties.filter((e3) => "color" === e3.type && e3.name === t2), i2);
    this.refresh();
  }
  get colors() {
    return this._colorsProxy || (this._colorsProxy = L.call(this)), this._colorsProxy;
  }
  set stroke(e2) {
  ...
  set state(e2) {
    ...
    this._state = void 0, t(e2) ? this._state = this._states.filter((e3) => e3.default)[0] : e2 && (this._state = this._states.filter((t2) => t2.name === e2)[0], this._state || (this._state = this._states.filter((e3) => e3.default)[0])), this._state ? this._lottie?.setSegment(this._state.time, this._state.time + this._state.duration + 1) : this._lottie.resetSegments(true), this.goToFirstFrame(), i2 && (this.pause(), this.play());
  }
  get state() {
    return this._state ? this._state.name : "";
  }
  set speed(e2) {
    this._speed = e2, this._lottie?.setSpeed(e2);
  }
  get speed() {
    return this._speed;
  }
  ...
  get rawProperties() {
    return this._rawProperties || (this._rawProperties = m(this._iconData, {
      lottieInstance: true
    }), !this._states.length && this._rawProperties && (this._rawProperties = this._rawProperties.filter((e2) => "scale" !== e2.name && "axis" !== e2.name && "stroke" !== e2.name && !e2.name.startsWith("state-")))), this._rawProperties || [];
  }
};
var D = class {
  player;
...
var T = class {
  ...
  onConnected() {
    for (const e2 of P) this.targetElement.addEventListener(e2.name, this.onClick, e2.options);
  }
  onDisconnected() {
    for (const e2 of P) this.targetElement.removeEventListener(e2.name, this.onClick);
  }
  onClick() {
...
var q = class {
  ...
  constructor(e2, t2, i2) {
    this.player = e2, this.element = t2, this.targetElement = i2, this.observer = new MutationObserver((e3, t3) => {
      for (const t4 of e3) "attributes" === t4.type && "sequence" === t4.attributeName && (this.reset(), this.step());
    });
  }
  onReady() {
    this.step();
  ...
  handleStep(e2, t2) {
    ...
    } else if ("state" === e2) this.frameState = t2[0], this.step();
    else if ("delay" === e2) {
      let e3 = null;
      for (const i2 of t2) i2 && i2.match(R) && (e3 = +i2);
    ...
    } else if ("idle" !== e2) throw new Error(`Invalid sequence action: ${e2}`);
  }
  step() {
    const {
      action: e2,
...
function O(e2) {
  I.setPlayerFactory((t2, i2, s2) => new w(e2, t2, i2, s2)), I.defineTrigger("in", F), I.defineTrigger("click", T), I.defineTrigger("hover", A), I.defineTrigger("loop", x), I.defineTrigger("loop-on-hover", M), I.defineTrigger("morph", S), I.defineTrigger("boomerang", D), I.defineTrigger("sequence", q), I.defineTrigger("morph-two-way", D), customElements.get && customElements.get("lord-icon") || customElements.define("lord-icon", I);
}
export {
  D as Boomerang,
  T as Click,
