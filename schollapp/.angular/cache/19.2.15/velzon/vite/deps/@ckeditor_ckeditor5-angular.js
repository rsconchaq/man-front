...
function createCKCdnBaseBundlePack({
  ...
  const urls = {
    scripts: [
      ...
      ...without(["en"], translations || []).map((translation) => createCKCdnUrl("ckeditor5", `translations/${translation}.umd.js`, version))
    ],
    stylesheets: [createCKCdnUrl("ckeditor5", "ckeditor5.css", version)]
  };
  return {
...
function createCKCdnPremiumBundlePack({
  ...
  const urls = {
    scripts: [
      ...
      // EN bundle is prebuilt into the main script, so we don't need to load it separately.
      ...without(["en"], translations || []).map((translation) => createCKCdnUrl("ckeditor5-premium-features", `translations/${translation}.umd.js`, version))
    ],
    stylesheets: [createCKCdnUrl("ckeditor5-premium-features", "ckeditor5-premium-features.css", version)]
  };
  return {
    // Preload resources specified in the pack, before loading the main script.
    preload: [...urls.stylesheets, ...urls.scripts],
...
function createCKBoxBundlePack({
  ...
  return __spreadProps(__spreadValues({
    // Load the main script of the base features.
    scripts: [createCKBoxCdnUrl("ckbox", "ckbox.js", version)]
  }, theme && {
